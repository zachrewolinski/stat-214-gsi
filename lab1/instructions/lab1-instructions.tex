...
\section*{Submission}
Push a folder called \texttt{lab1} to your \texttt{stat-214 GitHub} repository by 23:59 on Friday, February 20th. I will run a script that will pull from each of your GitHub repositories promptly at midnight so take care not to be late as late labs will not be accepted.
...
\subsection*{Special Coding Instructions for Lab 1}
...
You also must have a \texttt{models.py} Python file in your \texttt{code} folder. I will not enforce function names in this file, since the methods you implement will not be the same, but I am expecting each of your models to be implemented in this file. The functions in \texttt{models.py} used to create these models should be included in \texttt{run.sh} (see general instructions) to recreate your results.

I will be using \texttt{ruff} to assess code style. For simplicity, I will use its default setup, so no special configuration will be needed. Specifically, I will be running \texttt{ruff check code/clean.py code/models.py --output-format json} over your codebase, with the exact style points being a function of the number of style errors detected. You can run this command to check your style yourself, and also use \texttt{ruff check code/clean.py code/models.py --fix} to automatically fix many of these errors.
...
\begin{itemize}
    ...
    \item Data modeling (discussion and figures). You will not be graded strictly on the accuracy of your models (of course, if your models perform very poorly, you should inspect that), but rather the manner in which you use them to learn about the data. 
...
You are allowed to use LLMs (ChatGPT, GitHub Copilot, Cursor, etc.) to \textit{assist} in this lab.
%
You are \textit{strongly advised} to not lean too heavily on language models in your data cleaning process.
%
Exploratory data analysis and data cleaning are both HIGHLY iterative and unstructured; thus, there is less high-quality instruction on this in pre-training data.
%
As a result, LLMs tend to be overly simplistic in their data cleaning efforts.
...
\section*{Part 1: Data Cleaning (85\% of Lab 1 Grade)}
\subsection*{Data}
...
\section*{Part 2: Modeling (15\% of Lab 1 Grade)}

\subsection*{Model Implementation}

Implement three different classification methods to determine whether or not a CT scan should be recommended. These models should be:
\begin{itemize}
    \item The clinical decision rule from Kuppermann et al.
    \item A logistic regression.
    \item A different model of your own choosing.
\end{itemize}
While the CDR should be what is described in Kuppermann, you have creative freedom to add small details to the logistic regression. For example, you can choose (not required) to add penalization, shift the decision threshold, etc.
%
The last model (of your own selection) can be as simple or complex as you would like - for example, the clinical decision rule in Kuppermann et al. is incredibly simple, yet it is the industry standard. Explain the details of your model, as well as the choice of any parameters that were selected.
